document.addEventListener("DOMContentLoaded", () => {
  ...
  const isProfilePage = window.isProfilePage === true;

  const AVATAR_KEY = "savr-avatar";

  const smallProfileIcon = document.querySelector(".profile-icon");
  const savedAvatar = localStorage.getItem(AVATAR_KEY);

  if (smallProfileIcon) {
    smallProfileIcon.src = savedAvatar
      ? `images/${savedAvatar}`
      : "images/blank-avatar.png";
  }

  // Streak dragon uses the same avatar as the small profile icon
  const streakDragonImg = document.getElementById("streakDragon");
  if (streakDragonImg) {
    streakDragonImg.src = savedAvatar
      ? `images/${savedAvatar}`
      : "images/blank-avatar.png";
  }

  // Bottom-nav "G" (Goal) button opens the Set Goal modal
  const navGoalBtn = document.getElementById("navGoalBtn");
  if (navGoalBtn) {
      navGoalBtn.addEventListener("click", () => {
  ...
  const GOAL_PRESETS = {
    example1: "Complete 3 No Spending Days in a 7 day period. +20XP",
    example2: "Achieve diamond level. +40XP",
    example3: "Spend an average of less than $20/day this week. +60XP"
  };
  ...
  const setGoalBtn = $("navGoalBtn"); // or some actual goal button you want


  // Bottom quick menu (opened by "+")
  const navPlusBtn = $("navAddBtn");
  const navInfoBtn = $("navInfoBtn");   
  const navAllowBtn = $("navAllowBtn");   
  const quickMenuOverlay = $("quickMenuOverlay");
  const quickMenuAddExpense = $("quickMenuAddExpense");
  const quickMenuFaves = $("quickMenuFaves");
  const quickMenuNoSpend = $("quickMenuNoSpend");

  const infoMenuOverlay = $("infoMenuOverlay");
  const infoMenuAnalyticsBtn = $("infoMenuAnalytics");
  const infoMenuLeaderboardBtn = $("infoMenuLeaderboard");
  const infoMenuMissionsBtn = $("infoMenuMissions");


  // Close when tapping the dim background
  if (quickMenuOverlay) {
    on(quickMenuOverlay, "click", (e) => {
      if (e.target === quickMenuOverlay) {
        closeQuickMenu();
      }
    });
  }
  ...
  if (!isProfilePage) {
    ...
    // Streak islands + dragon visual
    const streakIslandsContainer = $("streakIslands");
    const streakDragon = $("streakDragon");
    ...
    on(hardResetBtn, "click", () => {
      ...
      // Visually reset small profile icon on this page
      if (smallProfileIcon) {
        smallProfileIcon.src = "images/blank-avatar.png";
      }
      ...
      if (mode === "weekly") {
        ...
        if (savingsInfoDiv) {
          savingsInfoDiv.textContent =
            `Saved: ${(weeklyAllowance - spentAll).toFixed(2)}`;
        }
      } else {
        const data = getMonthData();
        ...
        if (savingsInfoDiv) {
          savingsInfoDiv.textContent =
            `Saved: ${(dailyAllowance - spentToday).toFixed(2)}`;
        }
      }
    ...
    function updateStreakIslandsVisual() {
      if (!streakIslandsContainer || !streakDragon) return;

      const islands = streakIslandsContainer.querySelectorAll(".streak-island");
      if (!islands.length) return;

      // Clamp streak between 0 and 7
      const rawStreak = Number(settings.streak || 0);
      const streak = Math.max(0, Math.min(7, rawStreak));

      // Highlight islands that are "covered" by the streak
      islands.forEach((el, index) => {
        const n = index + 1;
        el.classList.toggle("active", n <= streak && streak > 0);
      });

      const containerRect = streakIslandsContainer.getBoundingClientRect();

      // Helper to place dragon above a given island element
      const placeAboveIsland = (islandEl) => {
        const r = islandEl.getBoundingClientRect();
        const centerX = r.left - containerRect.left + r.width / 2;
        const topY = r.top - containerRect.top - 40; // 40px above island
        streakDragon.style.left = `${centerX}px`;
        streakDragon.style.top = `${topY}px`;
        streakDragon.style.opacity = "1";
      };

      // No streak yet – park dragon above island 1 (fully on-screen)
      if (streak <= 0) {
        placeAboveIsland(islands[0]);
        return;
      }

      // Place dragon above the island matching the streak (1–7)
      const targetIndex = Math.min(streak, islands.length) - 1;
      placeAboveIsland(islands[targetIndex]);
    }
    ...
    function renderForCurrentMonth() {
      ...
      if (mode === "weekly") {
        allowanceDisplay.textContent = `Allowance: ${weeklyAllowance.toFixed(2)}`;
      } else {
        allowanceDisplay.textContent = `Allowance: ${dailyAllowance.toFixed(2)}`;
      }
    ...
    function maybeAwardAllowanceXP() {
      ...
      // If there was no bonus (first day of streak or no extension),
      // move the dragon immediately. For extended streaks, the move
      // will be synced with the red streak popup instead.
      if (bonusXP === 0) {
        updateStreakIslandsVisual();
      }
    ...
    function showStreakPopup(streak, bonusXP) {
      // Move dragon onto the new island at the same moment
      // the red streak popup appears.
      updateStreakIslandsVisual();
    ...
    // Bottom-nav "A" (Allowance) button opens the Set Allowance modal
    if (navAllowBtn) {
      navAllowBtn.addEventListener("click", () => {
        openAllowanceModal();
      });
    }
    ...
    function openInfoMenu() {
      if (infoMenuOverlay) infoMenuOverlay.style.display = "flex";
    }
    function closeInfoMenu() {
      if (infoMenuOverlay) infoMenuOverlay.style.display = "none";
    }
    ...
    if (infoMenuOverlay) {
      infoMenuOverlay.addEventListener("click", (e) => {
        if (e.target === infoMenuOverlay) {
          closeInfoMenu();
        }
      });
    }
    ...
    // Quick menu actions
    if (quickMenuAddExpense) {
      quickMenuAddExpense.addEventListener("click", () => {
        closeQuickMenu();
        // reuse your existing "add" flow
        openAddTypeModal();
      });
    }

    if (quickMenuFaves) {
      quickMenuFaves.addEventListener("click", () => {
        closeQuickMenu();
        openFavesModal();
      });
    }
    ...
    if (quickMenuNoSpend) {
      quickMenuNoSpend.addEventListener("click", () => {
        closeQuickMenu();
        if (noSpendBtn) {
          noSpendBtn.click();
        }
    ...
    // Info menu actions
    if (infoMenuAnalyticsBtn) {
      infoMenuAnalyticsBtn.addEventListener("click", () => {
        closeInfoMenu();
        openAnalyticsModal();
      });
    }

    if (infoMenuLeaderboardBtn) {
      infoMenuLeaderboardBtn.addEventListener("click", () => {
        closeInfoMenu();
        openLeaderboardModal();
      });
    }

    if (infoMenuMissionsBtn) {
      infoMenuMissionsBtn.addEventListener("click", () => {
        closeInfoMenu();
        openRewardsModal();
      });
    }
    ...
    // "+" button → open/close quick bottom sheet
    on(navPlusBtn, "click", () => {
      if (!quickMenuOverlay) {
        // Fallback: if overlay missing, just behave like "Add expense"
        openAddTypeModal();
        return;
      }

      if (quickMenuOverlay.style.display === "flex") {
        closeQuickMenu();
      } else {
        // make sure the info sheet isn't open at the same time
        closeInfoMenu();
        openQuickMenu();
      }
    });

    // "i" button → open/close info bottom sheet
    on(navInfoBtn, "click", () => {
      if (!infoMenuOverlay) return;

      if (infoMenuOverlay.style.display === "flex") {
        closeInfoMenu();
      } else {
        // don't stack both sheets
        closeQuickMenu();
        openInfoMenu();
      }
    });
  ...
  }// <-- end if (!isProfilePage)

  /* ---------- Profile page logic ---------- */
  const nameInput = $("nameInput");
  const saveNameBtn = $("saveNameBtn");
  const profileBigIcon = document.getElementById("profileBigIcon");
  const avatarPicker = $("avatarPicker");
  const avatarEditBtn = $("avatarEditBtn");
  const avatarOptionImgs =
    avatarPicker ? avatarPicker.querySelectorAll("[data-avatar]") : null;
  ...
  if (nameInput && saveNameBtn) {
    ...
    if (profileBigIcon && storedAvatar) {
      profileBigIcon.src = `images/${storedAvatar}`;
      profileBigIcon.dataset.avatar = storedAvatar;
    }

    // Open/close avatar picker
    const openAvatarPicker = () => {
      if (avatarPicker) avatarPicker.style.display = "block";
    };
    ...
    if (profileBigIcon) {
      profileBigIcon.style.cursor = "pointer";
      on(profileBigIcon, "click", openAvatarPicker);
    }
    if (avatarEditBtn) {
      on(avatarEditBtn, "click", openAvatarPicker);
    }

    if (avatarPicker && avatarOptionImgs) {
      avatarOptionImgs.forEach((img) => {
        on(img, "click", () => {
          const file = img.dataset.avatar;
          if (!file || !profileBigIcon) return;
          profileBigIcon.src = `images/${file}`;
          profileBigIcon.dataset.avatar = file;
          closeAvatarPicker();
        });
      });
    }

    // When Update is clicked: save name + avatar, then go home
    on(saveNameBtn, "click", () => {
      const name = nameInput.value.trim();
      localStorage.setItem("username", name);

      if (profileBigIcon && profileBigIcon.dataset.avatar) {
        localStorage.setItem(AVATAR_KEY, profileBigIcon.dataset.avatar);
      }
